...
foreach ($f in (Get-ChildItem -Path $srcDir -Filter *.md | Where-Object { $_.Name -ne "index.md" } | Sort-Object Name)) {
    ...
    # status: szkic — pomijamy (nie publikujemy), ale tylko dla nowych artykulow (bez pola zrodlo)
    $status = ""
    if ($raw -match '(?m)^status:\s*"?([^"\r\n]+?)"?\s*$') { $status = $matches[1].Trim().ToLower() }
    $hasZrodlo = $raw -match '(?m)^zrodlo:'
    if ($status -eq "szkic" -and -not $hasZrodlo) { continue }
